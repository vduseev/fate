...
function launch_tests() {
    ...
    for i in "${!pairs[@]}"; do
        ...
        info "Launching test pair ($counter/${#pairs[@]}) $test_name..."

        # Launch docker container in the background
        local start_time=$(date +%s)
        
        if launch_container "$lang_spec" "$src_path" "$i"; then
            # If container has been sucessfully launched, then
            # add it to the monitored collection
            local cid=$(cat $FATE_LAUNCH_STATE_DIR/$test_name)
            runs["$i"]="$cid"
            times["$i"]="$start_time"
        else
            debug "Container has not been launched for $test_name"
        fi
